...
const columns = [
  ...
  { id: 'hatchRate', label: 'Hatch rate' },
  { id: 'eggsAmount', label: 'Eggs amount' },
  { id: 'larvaeQuantity', label: 'Larvae quantity (seeding)' },
  { id: 'eggsDensity', label: 'Eggs density' },
  { id: 'larvaeDensity', label: 'Larvae density (seeding)' },
];
...
function HatchRate() {
  ...
  useEffect(() => {
    (async () => {
      const res = await DataFetcher.getHatchRate();
      if (res && res.data) {
